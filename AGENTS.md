...
- `pay.nister.org` is the payment and admin portal. It may serve
  `config_public.php` and may be a fallback for status, but it must not become
  the primary API host for captive portal service calls.
...
- Keep captive `status.html` trying `api.nister.org` before `pay.nister.org`.
