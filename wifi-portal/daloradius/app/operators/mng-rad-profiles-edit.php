<?php
// PHP8: init $editCounter safely
if(!isset($editCounter)||!is_numeric($editCounter)){$editCounter=0;}
$editCounter=(int)$editCounter;

// PHP8 guard: init missing counters early (runs before any HTML)
if (!isset($editCounter) || !is_numeric($editCounter)) { $editCounter = 0; }
$editCounter = (int)$editCounter;
?>
<?php
...
// PHP8 safety: init counters used by the attributes UI
if (!isset($editCounter) || !is_int($editCounter)) { $editCounter = 0; }
if (!isset($addCounter)  || !is_int($addCounter))  { $addCounter  = 0; }
/*
 *********************************************************************************************************
...
// PHP8-safe: load config and check operator perms
if (!isset($configValues) || !is_array($configValues)) {
    $cfg = dirname(__DIR__, 2) . "/library/daloradius.conf.php";
    if (is_readable($cfg)) { include_once $cfg; }
}
$__chk = __DIR__ . "/include/check_operator_perm.php";
if (is_readable($__chk)) { include_once $__chk; }
    ...
    function print_edit_form($dbSocket, $profile_name, $table_name, $no_attributes_message) {
        ...
        if ($numrows > 0) {
            ...
            while ($row = $res->fetchRow()) {
                ...
                if (!isset($editCounter) || !is_numeric($editCounter)) { $editCounter = 0; }
                  $name = sprintf('editValues%s[]', (isset($editCounter) ? (int)$editCounter : 0));
                ...
                $editCounter = 1 + (isset($editCounter)?(int)$editCounter:0);
            }
...
// PHP8 guard for form row counter(s)
if (!isset($editCounter) || !is_numeric($editCounter)) { $editCounter = 0; }
$editCounter = (int)$editCounter;
